                            ...
                            if ([posts objectForKey:post_id]) {
                                ...
                                if ([post objectForKey:@"likes"]) {
                                    ...
                                    if ([likes objectForKey:snap.key]) {
                                        ...
                                        [(AppDelegate *)[[UIApplication sharedApplication] delegate] updateMyApplications:app_id :newData];
                                    }
                                }
                            }
                        }
    ...
    switch (alertView.tag) {
        case -999:
            if (buttonIndex == 0){
                ...
                NSString *child = [NSString stringWithFormat:@"%@%@/my_applications/%@/posts/%@/", [[Configs sharedInstance] FIREBASE_DEFAULT_PATH],[[Configs sharedInstance] getUIDU], app_id, aKey];
                ...
                [[ref child:child] removeValueWithCompletionBlock:^(NSError * _Nullable error, FIRDatabaseReference * _Nonnull ref) {
                    if (error == nil) {
                        ...
                        if ([data objectForKey:@"posts"]) {
                            NSMutableDictionary *posts = [[data objectForKey:@"posts"] mutableCopy];
                            if ([posts objectForKey:post_id]) {
                                ...
                                [newF addEntriesFromDictionary:data];
                                [newF removeObjectForKey:@"posts"];
                                [newF setObject:posts forKey:@"posts"];
                                
                                data = newF;
                                
                                [(AppDelegate *)[[UIApplication sharedApplication] delegate] updateMyApplications:app_id :data];
                            }
                        }
                        [self reloadData:nil];
                    }
...
- (void) reloadData:(NSNotification *) notification{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSData *application_data =  [[[myapplicationsRepo get:app_id] objectAtIndex:[myapplicationsRepo.dbManager.arrColumnNames indexOfObject:@"data"]] dataUsingEncoding:NSUTF8StringEncoding];
        
        data = [NSJSONSerialization JSONObjectWithData:application_data options:0 error:nil];
