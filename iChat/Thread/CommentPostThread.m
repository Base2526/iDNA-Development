...
@implementation CommentPostThread
-(void)start: (NSString *)nid: (NSString *)nid_item: (NSString *)text
{
    ...
    NSURL *url =@"";// [NSURL URLWithString:[NSString stringWithFormat:@"%@%@.json",  [Configs sharedInstance].API_URL, [Configs sharedInstance].COMMENT_POST ]];
    ...
    /*
     a_id : ย่อมาจาก application id
     p_id : ย่อมาจาก post id
     */
    NSString *dataToSend = [[NSString alloc] initWithFormat:@"uid=%@&a_id=%@&p_id=%@&text=%@", [[Configs sharedInstance] getUIDU], nid, nid_item, text];
